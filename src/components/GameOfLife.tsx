...
function generatePatternGrid(size: number, pattern: 'pulsar' | 'pentadecathlon' | 'gliderGun'): boolean[][] {
  const grid = Array(size).fill(null).map(() => Array(size).fill(false))
  ...
  switch (pattern) {
    case 'pulsar': {
      ...
      const points = [
        // Top
        [-2,1], [-2,2], [-2,3], [-1,6], [0,6], [1,6], [1,3], [1,2], [1,1],
        [-1,-6], [0,-6], [1,-6], [-2,-3], [-2,-2], [-2,-1], [1,-3], [1,-2], [1,-1],
        // Bottom (mirror of top)
        [2,1], [2,2], [2,3], [6,1], [6,0], [6,-1], [-1,3], [-1,2], [-1,1],
        [6,-1], [6,0], [6,1], [2,-3], [2,-2], [2,-1], [-1,-3], [-1,-2], [-1,-1],
      ]
      points.forEach(([y, x]) => {
        const row = center + y
        const col = center + x
    ...
    case 'gliderGun': {
      ...
      const points = [
        ...
        [8,3], [8,4], [9,2], [9,4], [10,2], [10,3], // Left ship
        [16,4], [16,5], [16,6], [17,4], [18,5], // Right ship
        [22,1], [22,2], [23,0], [23,2], [24,0], [24,1], // Right block
        [34,2], [34,3], [35,2], [35,3], // Far right block
      ]
      points.forEach(([y, x]) => {
...
export function GameOfLife({ gridSize, cellSize, tickCount, label, resetKey, pattern, highlight = false, highlightColor = 'emerald' }: GameOfLifeProps) {
  ...
  const computeNextGeneration = useCallback((currentGrid: boolean[][]) => {
  ...
  return (
    <div className={`relative p-4 bg-gray-900 rounded-lg transition-colors ${
      highlight ? `${colorMap[highlightColor as keyof typeof colorMap].bgHighlight} ${colorMap[highlightColor as keyof typeof colorMap].ring}` : ''
    }`}>
      <div className="flex justify-between items-center mb-2">
        <div className={`text-sm font-mono ${
          highlight ? colorMap[highlightColor as keyof typeof colorMap].text : 'text-gray-400'
        }`}>{label}</div>
        <div className={`text-sm font-mono ${
          highlight ? colorMap[highlightColor as keyof typeof colorMap].text : 'text-gray-400'
        }`}>Gen {tickCount}</div>
